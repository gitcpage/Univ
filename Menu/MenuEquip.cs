...
namespace Univ.NsMenu
{
  internal class MenuEquip
  {
    ...
    public void Notify(NotifyCode notifyCode)
    {
      switch (notifyCode)
      {
        case NotifyCode.Ok:
          ...
          else
          {
            ...
            if (equiped != arrowEqId)
            {
              ...
              JsTrans.console_log("Equip " + arrowEqId.ToString());
              charsWritable_[topArrowSelect_].ResetStatus();
              Data.Bag bag = Data.Bag.Instance;
              //JsTrans.Assert(bag.body(0) <= 1, "bag.body(0) <= 1 2");
              if (before >= 0)
              {
                bag.equipPlus((Data.EquipCategory)secArrowSelect_, before);
                JsTrans.console_log("Bag Plus " + before.ToString());
              }
              //JsTrans.Assert(bag.body(0) <= 1, "bag.body(0) <= 1 3");
              if (arrowEqId >= 0)
              {
                bag.equipPlus((Data.EquipCategory)secArrowSelect_, arrowEqId, -1);
                JsTrans.console_log("Bag Minus " + arrowEqId.ToString());
              }
              //JsTrans.Assert(bag.body(0) <= 1, "bag.body(0) <= 1 4");
              menuStatus_.EquipChange(topArrowSelect_);
              UpdateView();
              tbSecArrows_[secArrowSelect_].Foreground = currentBrush_;
            }
    ...
    void UpdateView()
    {
      ...
      // ▲▲▲はずす▲▲▲
      tbItemArrows_ = new TextBlock[/*eqs.Length + */1];
      ...
      for (int i = 0; i < eqs.Length; i++)
      {
        ...
        JsTrans.console_log("viewPos"+ viewPos+ ", i" + i);
    ...
    public void Create()
    {
      ...
      g.Children.Add(view_);
      //▼▼▼ビュー（装備選択）▼▼▼

      menuStatus_.Title("現", "新");
      menuStatus_.EquipChange(topArrowSelect_);
    }
  }
}
